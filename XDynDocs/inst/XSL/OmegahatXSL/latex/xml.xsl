...
\end{<xsl:value-of select="$codeName"/>}
<xsl:if test="not(local-name(following-sibling::* | following-sibling::text()[1]) = '') or 
 not(starts-with(following-sibling::*[1], '&#10;'))">::  <xsl:value-of select="not(local-name(following-sibling::*[1]) = '')"/><xsl:text>&#10;</xsl:text></xsl:if></xsl:template>
<!--  -->
